...
class BrushTicket(object):
                    ...
                    for car_no in self.numbers:
                        ...
                        if current_tr:
                            ...
                            if current_tr.find_by_tag('td')[self.seat_type_index].text == '--':
                                ...
                                piao_type = str(current_tr.find_by_tag('td')[self.seat_type_index].text)
                                if piao_type == '候补':
                                    print('%s  需要候补' % (car_no + '(' + current_time + ')',))
                                    sleep(0.2)
                                    continue
                                # 有票，尝试预订
                                ...
                                print('预订成功，请及时前往支付……')
                                # 发送通知信息
                                self.send_email(self.receiver_email, '恭喜您，抢到票了，请及时前往12306支付订单！')
    ...
    def send_email(self, receiver_addresss, contentss):
        # 第三方 SMTP 服务
...
def init_info():
    ...
    while seat_type == '':
        seat_type = input('座位类型不能为空，请重新输入：')
    # 抢票成功，通知该手机号码
    receiver_mobile = '15566264073'
    mobile_pattern = re.compile(r'^1{1}\d{10}$')
    while receiver_mobile == '' or re.findall(mobile_pattern, receiver_mobile) == []:
        receiver_mobile = input('预留手机号码不能为空或者格式不正确，请重新输入：')
    receiver_email = '3252689175@qq.com'
    while receiver_email == '':
        receiver_email = input('预留邮箱不能为空，请重新输入：')
    return passengers, from_time, from_station, to_station, numbers, seat_type, receiver_mobile, receiver_email


if __name__ == '__main__':
    passengers, from_time, from_station, to_station, numbers, seat_type, receiver_mobile, receiver_email = init_info()
    # 开始抢票
